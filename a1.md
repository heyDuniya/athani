...
3. [Hexagonal in practice — a Mercury walkthrough](#3-hexagonal-in-practice--a-mercury-walkthrough)
   ...
   - 3.3 [Deep dive: the Execution service](#33-deep-dive-the-execution-service)
   - 3.4 [The async-response insight](#34-the-async-response-insight)
   - 3.5 [The same pattern across the other services](#35-the-same-pattern-across-the-other-services)
...
**3. Mercury has many inbound channels, and hex handles multiplicity naturally.**
We have orders from Omni (Kafka), events between Mercury services (Kafka), execution reports from Cameron (ASA inbound socket), trader actions from BFF (REST/WS), and eventually FIX messages from brokers. Each is its own inbound adapter. All of them call into the same use cases. We do not write duplicate business logic per channel.
...
|---|---|---|
| `oms-core` | Trading team (us) | Order lifecycle — validation, compliance, state machine, persistence. Also surfaces UIs for trading ops via BFF. |
| `execution` | Trading team (us) | Routes orders to Cameron via its ASA socket connector; receives execution reports on a separate inbound socket. |
| `ticketing` | Trading team (us) | **Ticket formation only** — decides how a filled order should be booked (allocations, instructions). Hands the formed ticket to the downstream ticketing system. Does **not** own the integrations to the books of record. |
| `bff` | Trading team (us) | Aggregates Mercury state and serves UI components for trader-facing screens. Built on CPP's micro-UI framework, so components compose with ones owned by other teams. |

**What sits around Mercury — and what we deliberately do not own.**

| System | Team | What it does | Relation to Mercury |
|---|---|---|---|
| **Cosmos** | Portfolio Management | Order entry platform with its own UI and its own BFF | Upstream — originates most orders, feeds them into Omni |
...
- The **`Execution ↔ Cameron`** interaction is two separate arrows, not one round-trip. That is deliberate and Section 3.4 explains why.
...
### 3.3 Deep dive: the Execution service
...
| `ProcessExecutionReportUseCase.process(ExecutionReport)` | Cameron sends us an execution report — fill, partial, reject, bust, or unsolicited update | Update execution state, publish fill events |

Operational use cases also exist (pause/resume routing to a venue, snapshot execution state for ops tooling), but the four above are the business core.
...
| Inbound | `ProcessExecutionReportUseCase` | Receive execution report |
| Outbound | `OrderRoutingPort` | Send order messages to a venue |
| Outbound | `ExecutionRepositoryPort` | Persist and retrieve executions |
...
### 3.4 The async-response insight
...
### 3.5 The same pattern across the other services
...
|---|---|---|---|
| `oms-core` | `Order`, `OrderLifecycle`, compliance rules | Kafka (from Omni, from execution, from ticketing), REST (ops) | Postgres, Kafka publisher, compliance-system client |
| `execution` | `Execution`, `Venue`, `Fill`, routing | Kafka (from oms-core), **ASA inbound socket (from Cameron)**, REST (ops) | **ASA outbound socket (to Cameron)**, Postgres, Kafka publisher |
| `ticketing` | `Ticket`, `Allocation`, ticket-formation rules | Kafka (from oms-core), REST (ops) | Handoff to downstream ticketing system, Postgres, Kafka publisher |
...
- `bff-application`: aggregation use cases (`GetBlotterForTrader`, `StreamLiveUpdates`).
- `bff-adapter`: inbound WS/REST from UI; outbound Kafka consumers and REST/gRPC clients to Mercury services.
...
- Cosmos has its own BFF, serving order-entry components. Mercury's BFF serves trading and execution views. Composition happens at the UI layer, across teams.
...
│   ├── execution-adapter/                ← CameronAsaInboundAdapter + CameronAsaOutboundAdapter
│   └── execution-service/
│
├── ticketing/
│   ├── ticketing-domain/
...
No Mercury service is simple. `ticketing` looks narrow today; it already has to form tickets correctly for multiple downstream paths (pricing via Bloomberg, investment book via Murex, accounting via SimCorp) and those rules will grow. By the time they do, the hex boundary will have paid for itself many times. Adding hex after the fact is strictly more expensive than having it from day one.
...
4. **Refactoring becomes expensive.** Restoring boundaries later means fighting a decade of accumulated coupling.
5. **Mercury becomes C3+ v2** — the exact failure mode we are rebuilding to avoid.
...
7. **Clear scope boundaries.** Omni and Cosmos are upstream (PM team); the downstream ticketing system is downstream (ticketing team); Mercury owns `oms-core`, `execution`, ticket *formation*, and trader-facing BFF.
8. **ArchUnit** as defense-in-depth for intra-module rules only.
9. **No inter-service or inter-team module dependencies.** All crossings go through published events and APIs.
