...
We have orders from Omni (Kafka), events between Mercury services (Kafka), execution reports from Cameron (ASA inbound socket), trader actions from BFF (REST/WS) — including manually-entered fills from voice trading — and eventually FIX messages from brokers. Each is its own inbound adapter. All of them call into the same use cases. **A Cameron fill and a voice-traded manual fill share zero adapter code but 100% of the domain logic** — that is exactly the separation hex is designed to deliver. See Section 3.3 for why this specific case is the cleanest hex example we have.
...
Without hex, fill-handling logic ends up split between "the Cameron path" and "the manual path," and over time those two paths drift. With hex, there is one path that all fills converge onto. That is the concrete payoff.
...
Notice that `ProcessExecutionReportUseCase` and `RecordManualFillUseCase` both culminate in the same domain operation: `Execution.applyFill(...)` followed by persistence and event publication. That convergence is the whole point.
...
No Mercury service is simple. Execution alone has five use cases spanning electronic routing, cancel, replace, electronic fill processing, and manual voice-trade fill recording — all of which must converge on the same domain operations. That is exactly the scenario hex is designed for. Adding hex after the fact is strictly more expensive than having it from day one.
